...
class TestOrchestratorInvoke:
    def test_invoke_simulated(self):
        ps, orch, _ = _setup()
        resp = orch.invoke(AIMode.GENERATE_ENTITY).value
        assert resp.provider == "simulated"
        assert len(resp.candidates) == 2

    def test_all_8_modes(self):
        ps, orch, _ = _setup()
        for mode in AIMode:
...
class TestOrchestratorGenerate:
    def test_generate_candidates(self):
        ps, orch, _ = _setup()
        cands = orch.generate_candidates(AIMode.GENERATE_ENTITY).value
        assert len(cands) == 2
    ...
    def test_generate_no_mutation(self):
        ...
        orch.generate_candidates(AIMode.GENERATE_ENTITY)
    ...
    def test_provider_error_handling(self):
        ...
        result = orch.invoke(AIMode.GENERATE_ENTITY)
        assert isinstance(result, Error) or result.value.error is not None

    def test_all_8_modes_generate(self):
        ps, orch, _ = _setup()
        for mode in AIMode:
