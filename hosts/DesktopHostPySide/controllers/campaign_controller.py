...
class CampaignController:
    ...
    def list_all(self):
        _apptrace(f"CTRL CampaignController.list_all"[:120])
        return self.cs.list_campaigns()

    def create(self, data):
        _apptrace(f"CTRL CampaignController.create data_keys={list(data.keys()) if isinstance(data, dict) else type(data).__name__}"[:120])
        return self.cs.create_campaign(data)

    def get(self, campaign_id):
        _apptrace(f"CTRL CampaignController.get campaign_id={campaign_id!r}"[:120])
        return self.cs.get_campaign(campaign_id)

    def overview(self, campaign_id):
        _apptrace(f"CTRL CampaignController.overview campaign_id={campaign_id!r}"[:120])
        return self.cs.get_campaign_overview(campaign_id)

    def update(self, campaign_id, data):
        _apptrace(f"CTRL CampaignController.update campaign_id={campaign_id!r}"[:120])
        return self.cs.update_campaign(campaign_id, data)

    def add_player(self, campaign_id, player_name):
        _apptrace(f"CTRL CampaignController.add_player campaign_id={campaign_id!r} name={player_name!r}"[:120])
        return self.cs.add_player(campaign_id, player_name)

    def create_clock(self, campaign_id, data):
        _apptrace(f"CTRL CampaignController.create_clock campaign_id={campaign_id!r}"[:120])
        return self.cs.create_clock(campaign_id, data)

    def advance_clock(self, clock_id, by=1):
        _apptrace(f"CTRL CampaignController.advance_clock clock_id={clock_id!r} by={by}"[:120])
